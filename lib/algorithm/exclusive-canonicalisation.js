...
ExclusiveCanonicalisation.prototype._renderNamespace = function _renderNamespace(node, prefixesInScope, defaultNamespace) {
  ...
  if (node.prefix) {
    if (newPrefixesInScope.indexOf(node.prefix) === -1) {
      nsListToRender.push({
        prefix: node.prefix,
        namespaceURI: node.namespaceURI,
      });

      newPrefixesInScope.push(node.prefix);
    }
  ...
  if (node.attributes) {
    for (var i=0;i<node.attributes.length;i++) {
      ...
      if (attr.prefix && newPrefixesInScope.indexOf(attr.prefix) === -1 && attr.prefix !== "xmlns") {
        nsListToRender.push({
          prefix: attr.prefix,
          namespaceURI: attr.namespaceURI,
        });

        newPrefixesInScope.push(attr.prefix);
